...
(** Which style is preferable in a given situation depends on
    ...
    present purposes---please stick with it for now on exercises. *)
...
(* ----------------------------------------------------------------- *)
(** *** Sums *)
(** Now that we can talk about lists of numbers, we can talk about the
    mathematical concept of sums.

   As a first draft, let's consider the following mathematical
   utterance:
   ...
   The capital greek letter sigma is just short for "sum". We say
   we'll be summing over a variable i, starting from 1 and going up to
   n. For each such i, we'll add i. That is,
   ...
   But we could phrase the sum differently: we're computing the sum of
   the list [0,1,2,...,n-1,n]. Here's a function that can sum a list:
*)
...
(** **** Exercise: 2 stars (sum_natlist_zeros)  *)

(** Define a function [zeros : nat -> natlist] that, given a natural
    [n], produces a list of [n] zeroes.

    Then prove a theorem (on paper, not in Coq) showing that for any [n],

    Σ_i=1^n 0 = 0

    i.e.,

    sum_natlist (zeros n) = 0.
*)

(* FILL IN HERE *)
(** [] *)

(** **** Exercise: 3 stars, recommended (sum_natlist_ntoone)  *)
...
<< 
    ...
    You'll have to do a _lot_ of arithmetic rewriting. Why not try it
    on the board, first? Don't forget about [assert]! *)
...
(** **** Exercise: 3 stars (sum_natlist_rev)  *)
(** You may have noticed that we didn't _exactly_ prove the sum we
    wanted to. First, we proved
    ...
    function we've written has been _total_, that is, defined on all
    inputs. Things have gotten a little bit weird for predecessor and
    division, but not hopelessly so. But defining division is pretty
    tricky---what should happen if we divide by zero? It's much easier
    to prove the version of the theorem that uses multiplication in Coq.

    There's another reason we didn't exactly prove the theorem we
    wanted to: the list of summands we provided goes from [n] down to
    [0], i.e., we've shown that [2 * (sum_natlist [n,n-1,...,2,1,0]) =
    n*(n+1)], but the informal statment used the list
    [0,1,2,...,n-1,n].

    Prove the following theorem, [sum_natlist_rev], showing that it
    doesn't matter what order we sum in. You may need to come up with
    a subsidiary lemma.
*)
...
(** What went wrong? Since [n] appeared more than once in the
    proposition we were proving---our goal---when we went by
    induction, we updated [n] in both cases.

    The solution here is _generalizing the induction hypothesis_: we
    prove a slightly more general property that will imply the one we
    want. Here, it's a simple generalization: separate out the number
    we're adding and the number of times we add it to itself. 
 *)

(** **** Exercise: 2 stars (sum_natlist_repeat_n)  *)
...
(** **** Exercise: 3 stars, advanced, optional (remove_decreases_count)  *)
...
(** **** Exercise: 3 stars (bag_count_sum)  *)
...
Definition example_map : partial_map := record (Id 5) 24 (record (Id 4) 23 empty).
...
Compute (find (Id 5) example_map).
Compute (find (Id 4) example_map).
Compute (find (Id 3) example_map).
...
(** $Date: 2017-11-15 22:44:48 -0800 (Wed, 15 Nov 2017) $ *)
