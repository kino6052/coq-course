...
(** **** Exercise: 2 stars (map_option)  *)
(** Define a function [map_option] that takes a function [f : X ->
    option Y] and a list of [a]s and returns a list of those [Y]s for
    which [f] didn't return [None]. Take [X] and [Y] as implicit
    arguments.

    Prove (in Coq) a lemma named [map_option_id] showing that
    [map_option (fun x => Some x) l = l] for all [l].  *)
...
(** Observe that the type of [fold] is parameterized by _two_ type
    ...
    Be concrete: say what your [X] and [Y] are. If you can, give a
    concrete function [f]! *)
...
(** **** Exercise: 2 stars (sum_sum')  *)
(** Define [sum : list nat -> nat] as a [Fixpoint] and [sum' : list
    nat -> nat] using a [fold]. Prove in Coq that they are
    equivalent. *)
...
(** **** Exercise: 2 stars (fold_app)  *)

Definition fold_app {X:Type} (l1 : list X) (l2 : list X) : list X
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.

(** Write down a theorem [fold_app_correct] in Coq stating that
   [fold_app] is correct, and prove it. *)
...
(** **** Exercise: 4 stars, advanced, optional (church_numerals)  *)
...
(** $Date: 2017-11-26 04:40:29 -0800 (Sun, 26 Nov 2017) $ *)
